...
test_term_query_impl( CuTest* tc, const char *index_dir )
{
    ...
    lcn_term_t* term;
    char* query_str;
    lcn_query_t* query, *clone, *tquery;
    lcn_hits_t* hits;
    ...
    LCN_TEST( lcn_term_query_create( &tquery, term, pool ) );

    LCN_TEST( lcn_index_searcher_create_by_path( &searcher, index_dir, pool ) );
    LCN_TEST( lcn_query_bitvector_create( &bitvector, tquery, searcher, NULL, pool ) );

    LCN_TEST( lcn_filtered_query_create( &query, bitvector, pool ));
    ...
    CuAssertStrEquals( tc, "<filter>", query_str );
    ...
    LCN_TEST( lcn_searcher_search( searcher, &hits, clone, NULL, pool ) );
...
CuSuite* make_filtered_query_suite()
{
