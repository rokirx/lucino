...
assert_same_stream ( CuTest* tc,
                     lcn_istream_t* expected,
                     lcn_istream_t* actual )
{
    CuAssertPtrNotNull(tc, expected);
    CuAssertPtrNotNull(tc, actual);
    
    unsigned int expected_size = lcn_istream_size ( expected );
    unsigned int test_size = lcn_istream_size ( actual );
    CuAssertIntEquals ( tc, expected_size, test_size );
    
    unsigned int expected_pos = lcn_istream_file_pointer ( expected );
    unsigned int test_pos = lcn_istream_file_pointer( actual );
                      ...
                      long seek_to )
{
    ...
    {
        LCN_TEST ( lcn_istream_seek( expected, seek_to ) );
        LCN_TEST ( lcn_istream_seek( actual, seek_to ) );
                           ...
                           lcn_istream_t *actual )
{
    ...
    // seek to middle
    point = lcn_istream_size ( expected ) / 2;
    assert_same_streams ( tc, expected, actual, point );
    ...
    // seek to end - 1
    point = lcn_istream_size ( expected ) - 1;
    assert_same_streams ( tc, expected, actual, point );
    
    // seek to the end
    point = lcn_istream_size ( expected );
    assert_same_streams ( tc, expected, actual, point );
...
test_single_file(CuTest* tc)
{
    ...
    for(i = 0; i < data_size; i++ )
    {
        ...
        lcn_istream_t *expected = NULL;
        lcn_istream_t *actual = NULL;
        ...
        LCN_TEST ( lcn_istream_close( expected ) );
        LCN_TEST ( lcn_istream_close( actual ) );
