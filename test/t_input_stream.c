...
TestCuReadInt(CuTest* tc)
{
    ...
    LCN_TEST( lcn_istream_read_int( in, &result ) );
    ...
    CuAssertTrue(tc, len == lcn_istream_size( in ) );
...
#define CHECK_VINT( x )                                                  \
    LCN_TEST( lcn_istream_read_vint( in, &result ));                \
    CuAssertIntEquals(tc, x, result )


static void
test_read_vint_impl (	CuTest* tc, lcn_istream_t *in )
{
...
#define CHECK_LONG( x )                                                 \
    LCN_TEST( lcn_istream_read_long( in, &result ));               \
    CuAssertTrue(tc, x == result )


static void
test_read_long_impl (CuTest* tc, lcn_istream_t *in )
{
    apr_int64_t result;
    CuAssertPtrNotNull(tc, in );
    CHECK_LONG( 1 );
...
TestCuReadString(CuTest* tc)
{
    ...
    LCN_TEST( lcn_istream_read_string( in, &buf, &len, p ) );
...
TestCuReadBytes(CuTest* tc)
{
    ...
    len = 8;
    LCN_TEST( lcn_istream_read_bytes( in, bytes, 7, &len ) );
...
TestCuClone(CuTest* tc)
{
    ...
    CuAssertIntEquals(tc, len, 26);
    CuAssertStrEquals(tc, buf, "Apache Software Foundation");
    LCN_TEST( lcn_istream_clone( in, &clone, p ) );
    LCN_TEST( lcn_istream_read_string( in, &buf, &len, p ) );
    CuAssertStrEquals(tc, buf, "Teste Sonderzeichen: ouml: ö, auml: ä, uuml: ü, Ouml: Ö, Auml: Ä, Uuml: Ü, szlig: ß" );
    LCN_TEST( lcn_istream_read_string( clone, &buf, &len, p ) );
    CuAssertIntEquals(tc, len, 26);
    CuAssertStrEquals(tc, buf, "Apache Software Foundation");
    LCN_TEST( lcn_istream_close( in ) );
