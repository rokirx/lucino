...
public class FieldInfos
{
    ...
    public static void main( String[] args ) throws Exception
    {
        ...
        for( int i = 0; i < data.length; i++ )
        {
            ...
            //doc.add( new Field( "tv_po", data[i], Field.Store.NO, Field.Index.NO, Field.TermVector.WITH_POSITIONS_OFFSETS ) );
            // -> it doesn't make sense to have a field that is neither indexed nor stored

            // doc.add( new Field( "tv_po_s", data[i], Field.Store.YES, Field.Index.NO, Field.TermVector.WITH_POSITIONS_OFFSETS ) );
            // -> cannot store term vector information for a field that is not indexed

            doc.add( new Field( "tv_po_i", data[i], Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.WITH_POSITIONS_OFFSETS ) );
            doc.add( new Field( "tv_po_is", data[i], Field.Store.YES, Field.Index.TOKENIZED, Field.TermVector.WITH_POSITIONS_OFFSETS ) );

            doc.add( new Field( "tv_p_i", data[i], Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.WITH_POSITIONS ));
            doc.add( new Field( "tv_p_is", data[i], Field.Store.YES, Field.Index.TOKENIZED, Field.TermVector.WITH_POSITIONS ));

            doc.add( new Field( "tv_o_i", data[i], Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.WITH_OFFSETS ));
            doc.add( new Field( "tv_o_is", data[i], Field.Store.YES, Field.Index.TOKENIZED, Field.TermVector.WITH_OFFSETS ));

            doc.add( new Field( "tv_i", data[i], Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.YES ));
            doc.add( new Field( "tv_is", data[i], Field.Store.YES, Field.Index.TOKENIZED, Field.TermVector.YES ));

            doc.add( new Field( "ntv_i", data[i], Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.NO ));
    ...
    private static String[] data = {
        "Lucene Change Log",
        "$Id: FieldInfos.java,v 1.1 2006/02/03 00:41:14 rokirchg Exp $",
        "1.3 final",
