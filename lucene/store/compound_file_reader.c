                                  ...
                                  apr_pool_t *pool )
{
    ...
    {        
        ...
        LCNCE( lcn_istream_read_vint( (*cfr)->istream, &count ));
        ...
        for ( i = 0; i < count; i++ )
        {
            ...
            LCNCE( lcn_istream_read_long( (*cfr)->istream, &offset ) );
            
            char *file_name = NULL;
            unsigned int len = 0;
            LCNCE ( lcn_istream_read_string( (*cfr)->istream, &file_name, &len, (*cfr)->pool ) );
        ...
        if (new_entry != NULL) 
        {
            unsigned int stream_size = lcn_istream_size( (*cfr)->istream );
    ...
    if ( s )
    {
        if ( (*cfr)->istream != NULL ) 
        {
            LCNCR ( lcn_istream_close( (*cfr)->istream ));
        }
    }
